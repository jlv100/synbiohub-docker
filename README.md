...
To run the configuration with SBOLExplorer, use `docker-compose -f synbiohub-docker.yml -f synbiohub-docker.explorer.yml up`.

To add iGEM rendering plugins, add the `docker-compose.igem.yml`.

To run the configuration with iGEM, use `docker-compose -f synbiohub-docker.yml -f synbiohub-docker.igem.yml up`.
